...
$totalSQL = "SELECT 
        p.post_uid,p.post_user_id,p.post_privilege
        FROM user u,blog_post p 
        WHERE p.post_user_id = u.id AND u.state='activated' AND p.post_privilege!='lock'
    ";
...
$sql = "SELECT 
    u.id, u.nickname, u.state, u.title, u.color, u.state,
